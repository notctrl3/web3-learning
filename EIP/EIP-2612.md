...
```
1. \x19\x01 是固定前缀，用来区分 EIP-191 风格签名

2. domainSeparator 是合约上下文信息（name/version/chainId/address）

3. structHash 是 typed struct 哈希（例如 Permit）
...
1. The current blocktime is less than or equal to deadline.
2. owner is not the zero address.
3. nonces[owner] (before the state update) is equal to nonce.
4. r, s and v is a valid secp256k1 signature from owner of the message:
